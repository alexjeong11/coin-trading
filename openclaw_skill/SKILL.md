...
1. **Hybrid A+B Grid Spacing**: The bot uses Dynamic ATR. Buy-side: first two slots are tight (0.25%) but lower slots scale exponentially (1.5x) to catch falling knives safely. Sell-side: first two slots hyper-scalp at 0.15%, but upper slots scale aggressively (2.5x) acting as a Core Holding that only sells on massive structural bull runs. This allows for high-frequency flipping in sideways markets and maximum profit taking ("home runs") during strong uptrends.
   ...
   - **Time-based Rebalancing (Zombie Grid Prevention)**: If no single trade happens for the duration of `RESET_TIMER_HOURS`, the bot usually cancels all current maker orders and recalculates the grid hugging the modern current price. **HOWEVER, [Uptrend Protection] applied:** If the current price is HIGHER than the original grid center (meaning the bot is simply holding coins and waiting to sell at a higher profit target during a bull market), it will SKIP the reset to prevent buying coins back at artificially inflated prices. It only resets in sideways or downtrends.
