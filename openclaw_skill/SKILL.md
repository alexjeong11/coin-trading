...
1. **Hybrid A+B Grid Spacing**: The bot uses Dynamic ATR. Buy-side: first two slots are tight (0.25%) but lower slots scale exponentially (1.5x) to catch falling knives safely. Sell-side: first two slots hyper-scalp at 0.15%, but upper slots scale symmetrically (1.5x) to increase capital turnover and trade more robustly during sideways market sweeps (previously 2.5x locked capital for too long). This allows for high-frequency flipping in sideways markets and sustained compounding during standard uptrends.
   ...
   - **Partial Trailing Rebalance (Zombie Slot Prevention)**: If no single trade happens for the duration of `RESET_TIMER_HOURS` (default: 1h), the bot performs a smart **Partial Rebalance**. It ONLY cancels and migrating idle KRW-held slots to directly below the current price to scalp sideways wicks. It strictly preserves all ETH-held slots exactly where they are. This flawlessly guarantees zero "forced sell-lows" or "buy-highs", safely netting scalp profits without compromising core swing bags.
