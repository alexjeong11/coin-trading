...
1. **Hybrid A+B Grid Spacing**: The bot uses Dynamic ATR. Buy-side: first two slots are tight (0.25%) but lower slots scale exponentially (1.5x) to catch falling knives safely. Sell-side: first two slots hyper-scalp at 0.15%, but upper slots scale aggressively (2.5x) acting as a Core Holding that only sells on massive structural bull runs. This allows for high-frequency flipping in sideways markets and maximum profit taking ("home runs") during strong uptrends.
   ...
   - **Time-based Rebalancing (Disabled)**: The time-based reset (`RESET_TIMER_HOURS`) has been practically permanently disabled (set to 9999 hours) because resetting the grid dynamically during a downtrend mathematically forces the bot to "sell low" (realize losses). The bot will now patiently ride the original grid parameters during sideways and downtrend markets without arbitrarily dumping coins at new, lower center lines. Out-of-bounds safety mechanisms still exist for catastrophic drops (-5%).
